...
namespace NBDecryptor_CMD
{
    ...
    public class CertHelper
    {
        ...
        public static string ImportCertificateWithPrivateKey(byte[] rawByte, string password)
        {
            SystemX509.X509Certificate2 cert = new SystemX509.X509Certificate2();
            cert.Import(rawByte, password, keySetFlags_);
        ...
        public static void BackupCertificatePFX(SystemX509.X509Certificate2 cert, string backupDir, string password)
        {
            // Attempt to backup before installing - don't want customer to use a
            // private key that was never backed up.
            byte[] cert_bytes = cert.Export(SystemX509.X509ContentType.Pfx, password);
            string backup_path = Path.Combine(backupDir, cert.BaseName() + ".pfx");
            File.WriteAllBytes(backup_path, cert_bytes);
        }
